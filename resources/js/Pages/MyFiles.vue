<template>
    <AuthenticatedLayout>
        ...
        <nav class="flex items-center justify-between p-1 mb-3">
            ...
            <div class="flex">
                <ShareFilesButton :all="allSelected" :ids="selectedIds" class="mr-2"/>
...
// Computed
/*
    selected.value ستكون قيمتها بهذا الشكل مثلا
    {
        15: true
        12: true
        17: false
    }
    Object.entries(selected.value) تحولها اى مصفوفة كالتالي
    [[15, true], [12, true], [17, flase]]
    filter(a => a[1]) تعيد العناصر المحددة فقظ كالتالي
    [[15, true], [12, true]]
    map(a => a[0]) تعيد العنصر الأول من المصفوفة كالتالي
    [15, 12]
*/
const selectedIds = computed(() => Object.entries(selected.value).filter(a => a[1]).map(a => a[0]))

function showOnlyFavourites () {
    let path = props.folder.path ? '\\' + props.folder.path :''
    let param = new URLSearchParams(window.location.search)
    let search = param.get('search') ?? ''
    // console.log(props.folder, path, search)
    if(onlyFavourites.value) {
        router.visit(route('myFiles')+ path +'?favourites=show' + (search?'&search='+search:''), {
            onSuccess: () => {
                onlyFavourites.value = true
            }
        })
    }else {
        router.visit(route('myFiles')+ path + (search?'?search='+search:''), {
            onSuccess: () => {
